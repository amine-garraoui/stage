...
def _resolve_value(value: str | None) -> Path | None:
    """Safely resolve a path while preventing directory traversal."""
    if not value:
        return None
    ...
    try:
        ...
        LOGGER.error(
            "Security: Path traversal attempt detected. Path %s is outside BASE_DIR %s",
            candidate,
            BASE_DIR,
        )
        return None
    except (OSError, RuntimeError) as error:
        LOGGER.error("Error resolving path %s: %s", candidate, error)
        return None
    ...
    if any(char in str(candidate) for char in ["*", "?"]):
        try:
            for match in sorted(glob.glob(str(candidate))):
                try:
                    Path(match).resolve().relative_to(base_resolved)
                    return Path(match)
                except ValueError:
                    LOGGER.warning(
                        "Security: Glob match %s outside BASE_DIR, skipping", match
                    )
            return None
        except OSError as error:
            LOGGER.error("Error processing glob pattern %s: %s", candidate, error)
            return None
...
def resolve_path(value: str | None) -> Path | None:
    """Public API for secure path resolution."""
