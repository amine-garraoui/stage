...
def previous_month(df: pd.DataFrame, mois: str) -> str | None:
    """Return the latest available month before the selected month."""
    months = [month for month in available_months(df) if month < mois]
    return months[-1] if months else None


def compare_month_to_previous(df: pd.DataFrame, mois: str) -> tuple[str | None, pd.DataFrame]:
    """Build a comparable KPI table for a month and its previous available month."""
    previous = previous_month(df, mois)
    metrics = [
        ("total_ouverts", "Demandes ouvertes", ""),
        ("total_fermes", "Demandes fermées", ""),
        ("ouverts_et_fermes_meme_mois", "Ouvertes et fermées", ""),
        ("delai_moyen_heures", "Délai moyen", " h"),
        ("satisfaction_moyenne", "Satisfaction globale", " %"),
        ("taux_participation", "Taux de participation", " %"),
    ]
    current = calculate_month_kpi(df, mois).as_dict()
    old = calculate_month_kpi(df, previous).as_dict() if previous else {}
    rows = []
    for key, label, suffix in metrics:
        current_value = current.get(key)
        previous_value = old.get(key)
        try:
            delta = float(current_value) - float(previous_value) if current_value is not None and previous_value is not None else None
            variation = (delta / abs(float(previous_value)) * 100) if delta is not None and float(previous_value) != 0 else None
        except (TypeError, ValueError, ZeroDivisionError):
            delta = variation = None
        rows.append({
            "Indicateur": label,
            "Mois précédent": previous or "N/A",
            "Mois sélectionné": mois,
            "Valeur précédente": previous_value,
            "Valeur sélectionnée": current_value,
            "Écart": delta,
            "Variation": variation,
            "Suffixe": suffix,
        })
    return previous, pd.DataFrame(rows)
